...
async function checkAndSeedStock() {
  try {
    ...
    console.log(`Found ${stockCount} stock records in database`);

    if (stockCount === 0) {
      console.log('No stock data found. Adding sample data...');
      ...
      console.log('Stock data already exists. Displaying current data:');
      const stocks = await Stock.find({}).lean();
      stocks.forEach((stock, index) => {
        console.log(`${index + 1}. ${stock.product} (${stock.companyId})`);
        console.log(`   Opening: ${stock.openingStock}, Purchases: ${stock.totalPurchases}, Sales: ${stock.totalSales}`);
        console.log(`   Closing: ${stock.closingStock}, Available: ${stock.stockAvailable}`);
        console.log(`   Agents: ${stock.agentStocks ? stock.agentStocks.length : 0}`);
        console.log('');
      });
    }
