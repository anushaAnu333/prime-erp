...
export default function StockDashboard() {
  ...
  const deliveryColumns = [
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex gap-2">
          <Link href={`/dashboard/stock/delivery/${row._id}/edit`}>
            <Button size="sm" variant="outline">
              Edit
            </Button>
          </Link>
          <Button 
            size="sm" 
            variant="outline"
            onClick={() => handleCompleteDelivery(row._id)}
          >
            Complete
          </Button>
        </div>
  ...
  const handleCompleteDelivery = async (id) => {
    try {
      await dispatch(completeDelivery(id)).unwrap();
      // Refresh the delivery stock data
      dispatch(fetchDeliveryStocks(deliveryFilters));
      alert('Delivery marked as completed successfully!');
    } catch (error) {
      console.error('Error completing delivery:', error);
      alert(`Error: ${error || 'Failed to complete delivery'}`);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
        ...
        <>
          ...
          <Card className="mb-6 p-6 bg-gradient-to-r from-green-50 to-emerald-50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-center">
              <div className="bg-white p-4 rounded-lg shadow-sm">
                <div className="text-2xl font-bold text-blue-600">
                  {formatNumber(deliverySummary.totalStockReceived || 0)}
                </div>
                <div className="text-sm text-gray-600">Stock Received</div>
              </div>
              <div className="bg-white p-4 rounded-lg shadow-sm">
                <div className="text-2xl font-bold text-red-600">
              ...
              <div className="bg-white p-4 rounded-lg shadow-sm">
                <div className="text-2xl font-bold text-indigo-600">
                  = {formatNumber(deliverySummary.totalClosingStock || 0)}
                </div>
                <div className="text-sm text-gray-600">Closing Stock</div>
              </div>
            </div>
      ...
      {activeTab === "main" && (summary.lowStockCount > 0 || summary.expiredCount > 0) && (
      ...
      {activeTab === "delivery" && (
        <Card className="p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <Link href="/dashboard/stock/delivery/create">
              <Button>Add Delivery Stock</Button>
            </Link>
          </div>
          ...
          <Table
            ...
            emptyMessage="No delivery stock data available"
          />
        </Card>
      )}
    </div>
